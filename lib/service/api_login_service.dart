...
class ApiService {
  ...
  Future<Directory> _resolveWritableDir() async {
    ...
    try {
      ...
      AppLogger.warning('IMG API fallback to systemTemp', e);
      return Directory.systemTemp;
    }
  }
  ...
  */
  ...
  Future<String> userProfileImage(User user, BuildContext context) async {
    ...
    try {
      ...
      AppLogger.info(
        'IMG API start role=$role persId=$persId idAb=$idAb userId=${user.userId} kIsWeb=$kIsWeb url=$url',
      );
      ...
      AppLogger.info(
        'IMG API response status=${response.statusCode} contentType=${response.headers['content-type']} bytes=${response.bodyBytes.length}',
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (kIsWeb) {
          // Sul web usiamo i bytes autenticati e li convertiamo in data-uri,
          // così il rendering non dipende da una seconda richiesta non autenticata.
          final encoded = base64Encode(imageData);
          AppLogger.info('IMG API web data-uri length=${encoded.length}');
          return 'data:image/jpeg;base64,$encoded';
        } else {
          final appDocDir = await _resolveWritableDir();
          String filePath =
          ...
          AppLogger.info(
            'IMG API local save path=$filePath exists=$fileExists bytes=${imageData.length}',
          );
        ...
        AppLogger.warning(
          'IMG API non-200 status=${response.statusCode} bodyPreview=${response.body.length > 120 ? response.body.substring(0, 120) : response.body}',
        );
        throw Exception('Error retrieving image: ${response.statusCode}');
      }
    } catch (e) {
      AppLogger.error('IMG API exception', e);
